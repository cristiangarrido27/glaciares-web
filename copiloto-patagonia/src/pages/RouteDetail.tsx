...
export default function RouteDetail() {
  ...
  const handleDownloadPdf = () => {
    generateItineraryPdf(
      ...
      route.summary,
      [
        { heading: 'Paradas sugeridas', lines: route.stops.map((s) => `${s.name}: ${s.note}`) },
        { heading: 'Advertencias', lines: route.warnings },
        {
          heading: 'Datos generales',
          lines: [
            `Distancia aproximada: ${route.distanceKm ?? 'según vía elegida'} km`,
            `Tiempo aproximado: ${route.drivingTime}`,
            `Salida recomendada: ${route.suggestedDeparture}`,
            `Fronteras involucradas: ${route.bordersInvolved.length ? route.bordersInvolved.join(', ') : 'Ninguna'}`,
          ],
        },
      ],
      t('warnings.general')
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 lg:px-8">
      <Seo title={route.name} description={route.summary} />
      <Breadcrumbs
      ...
      <p className="mt-2 text-lg text-rock/80">{route.summary}</p>
      ...
      <dl className="mt-6 grid grid-cols-2 gap-4 rounded-2xl border border-slate-200 bg-white p-6 sm:grid-cols-3">
        <div>
          <dt className="text-xs font-bold uppercase text-rock/50">Kilometraje</dt>
          <dd className="mt-1 text-sm font-semibold text-glacial-dark">{route.distanceKm ?? 'Según vía elegida'} km</dd>
        </div>
        <div>
          <dt className="text-xs font-bold uppercase text-rock/50">{t('common.approxTime')}</dt>
          <dd className="mt-1 text-sm font-semibold text-glacial-dark">{route.drivingTime}</dd>
        </div>
        <div>
          <dt className="text-xs font-bold uppercase text-rock/50">Salida recomendada</dt>
          <dd className="mt-1 text-sm font-semibold text-glacial-dark">{route.suggestedDeparture}</dd>
        </div>
        <div className="col-span-2 sm:col-span-3">
          <dt className="text-xs font-bold uppercase text-rock/50">Fronteras involucradas</dt>
          <dd className="mt-1 text-sm font-semibold text-glacial-dark">
            {route.bordersInvolved.length ? route.bordersInvolved.join(', ') : 'Ninguna'}
          </dd>
        </div>
      </dl>

      <section className="mt-8">
        <h2 className="font-display text-xl font-extrabold text-glacial-dark">Paradas y puntos de interés</h2>
          ...
          {route.stops.map((stop) => (
            <li key={stop.name} className="flex items-start gap-3 p-4">
              ...
              <div>
                ...
                <p className="text-xs text-rock/70">{stop.note}</p>
              </div>
      ...
      <section className="mt-8">
        <h2 className="font-display text-xl font-extrabold text-glacial-dark">Advertencias</h2>
